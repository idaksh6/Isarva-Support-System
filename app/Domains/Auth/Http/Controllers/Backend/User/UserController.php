...
class UserController
{
    ...
    public function index(Request $request)
    {
        ...
        if(!empty($request['search'])){
            $user_name = User::where(DB::raw('concat(last_name," ",first_name)'),'like','%'.$request['search'].'%')
                ->get(['users.*']);
            $user_email = User::where('email','like','%'.$request['search'].'%')
                ->get(['users.*']);
        }
        ...
        if(!empty($request['search'])){
            ...
            $repository = $this->userService
                ->whereIn('id', $filtered_users);
        }

        return view('backend.auth.user.index',compact('search'))
            ->withUsers($repository
                ->orderBy('id', 'asc')
                ->paginate(10));
